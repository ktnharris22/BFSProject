...
//  asked questions to Nathan Saslavsky and Prof Goss
...
namespace ParallelBFS {
	...
	void WorkerThread(SharedLList<uint32_t> *workQueue, std::mutex *dataLock, uint8_t *data, uint32_t *seenStates, int depth){
		...
		{
			uint32_t start;
			//gets starting point from workQueue
			while (workQueue->RemoveFront(start)){
				if (start == ~0) {
					return;
				}
				LList <int> moves;
				SlidingPuzzleState s;
				LList <uint32_t> d; //rank array 
				//Goes through the workqueue
				for (int k = start; k < start + workSize && k < s.GetMaxRank(); k++){
					if (data[k] == depth){
						s.Unrank(k); //unranks the index to get the current state
						s.GetMoves(moves); //gets the moves in current state
						while (moves.IsEmpty() == false){
							s.ApplyMove(moves.PeekFront()); //apply moves to state
							uint32_t rank = s.Rank(); //ranks state
							s.UndoMove(moves.PeekFront());
							moves.RemoveFront();
							d.AddFront(rank); //puts rank in array 
						}
					}
				}
				//locks d access
				dataLock->lock(); 
				while (!d.IsEmpty()) {
					uint32_t rank = d.PeekFront(); //finds first rank in array d
					d.RemoveFront(); 
					//checks if rank is unseen
					if (data[rank] == 255) {
						data[rank] = depth + 1; //sets state to depth+1
						(*seenStates)++; //increments seen states
					}
				}
				dataLock->unlock();
			}
	...
	void DoBFS(int numThreads)
	{
		...
		while (seenStates != s.GetMaxRank())
		{
			...
			for (int i = 0; i < numThreads; i++){
				...
				int stop = std::min((i + 1)*workSize, int(s.GetMaxRank()));
				threads[i] = new std::thread(WorkerThread, &workQueue, &lock, stateDepths, &seenStates, currDepth);
			}

			//add to workQueue
			for (int j = 0; j < s.GetMaxRank(); j+=10000){
				workQueue.AddBack(j);
			}

			//signal to stop work queue
